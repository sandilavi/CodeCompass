...
export default function Quiz() {
  ...
  const handleNextQuestionClick = () => {
    ...
    if (nextQuestion < questions.length) {
      ...
      axios.post('https://codecompass-orjp.onrender.com/send_quiz_data', quizData)
        .then((response) => {
  ...
  const handleGetProficiencyLevel = async () => {
    try {
      const response = await axios.post('https://codecompass-orjp.onrender.com/send_quiz_data', {
